...
export const ProductSearch = ({ lines, onAddLine }: Props) => {
  ...
  const handleSelectProduct = (product: ProductsSearch) => {
    ...
    (product.variants ?? []).forEach((v) => {
      ...
      inputs[v.id] = {
        qty: existing ? existing.orderedQty.toString() : "",
        cost:
          existing?.costPerUnit !== undefined
            ? existing.costPerUnit.toString()
            : "",
      };
    });
  ...
  const handleAddLine = (variant: productSearchVariant) => {
    ...
    if (qty < 1) return;
    const costRaw = input?.cost ? parseFloat(input.cost) : undefined;
    const cost = costRaw !== undefined && !isNaN(costRaw) ? costRaw : undefined;
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    ...
                    <th className="px-4 py-2 font-medium text-muted-foreground w-64">
                      Cost / Unit (Optional)
                    </th>
